...
let DelegationController = class DelegationController {
    ...
    async getSerchDelegationByText(text) {
        return await this.delegationService.getSerchDelegationByText(text);
    }
    ...
    async updateDelegation(delegationDTO, id) {
        const task = await this.delegationService.updateDelegation(id, delegationDTO);
        return task ? task : { statusCode: '404', message: 'Delegation not found', error: "Not found" };
    }
    async deleteDelegation(id) {
        const { affected } = await this.delegationService.deleteDelegation(id);
        return affected == 0
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], DelegationController.prototype, "getSerchDelegationByText", null);
__decorate([
    (0, swagger_1.ApiResponse)({
        status: 200,
        type: delegation_dto_1.DelegationDTO
